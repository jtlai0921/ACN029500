...
Public Class F_TableGrade
    ...
    Private Sub F_TableDeptt_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Dim Msqlstr_1 As String = "Select * From TAB_GRADE"
    ...
    Private Sub DataGridView1_SelectionChanged(sender As Object, e As EventArgs) Handles DataGridView1.SelectionChanged
        ...
        T_Grade.Text = Strings.Trim(DataGridView1.CurrentRow.Cells(0).Value)
    End Sub
    ...
    Private Sub B_ADD_Click(sender As Object, e As EventArgs) Handles B_ADD.Click
        ...
        Dim Mgrade As String = Strings.Trim(T_Grade.Text)

        ' 第一段 ************************************************************************************
        ' 檢查『等級』是否已輸入，此項不得空白
        If Strings.Len(Mgrade) = 0 Then
            MsgBox("『等級』未輸入！", 0 + 16, "Error")
            Exit Sub
        End If

        ' 第二段 ************************************************************************************
        ' 檢查『等級』是否已存在，可使用 Find 方法 或 RowFilter 屬性（如下）
        ' 檢查 DataTable 即可（載入本表單時，已將全部資料存入 Table01）

        ' 使用 Find 方法，尋找欄須經排序
        'Dim O_DV As DataView = New DataView(ODataSet_1.Tables("Table01"))
        'O_DV.Sort = "grade ASC"
        'Dim Mcheck As Int32 = O_DV.Find(Mgrade)
        'If Mcheck <> -1 Then
        'MsgBox("Sorry, 『等級』已存在，不能新增相同的代號！" + Chr(13) + Chr(13) + "請重新輸入", 0 + 16, "Error")
        'Exit Sub
        'End If

        ' DataView 檢視表的 RowFilter 屬性篩選出 User 所輸入的等級，若有，則表示重複了
        ' 比較值 T_Title.Text 的前後要加單引號
        Dim O_DV As DataView = New DataView(ODataSet_1.Tables("Table01"))
        O_DV.RowFilter = "grade='" + Mgrade + "'"
        ...
        ' 宣告變數儲存 SQL 指令
        Dim Msqlstr_1 As String = "Insert into TAB_GRADE(grade) values(@t1)"
        ...
        Ocmd_1.Parameters.AddWithValue("@t1", SqlDbType.NVarChar).Value = Mgrade
        'Ocmd_1.Parameters.AddWithValue("@t1", Mgrade)
    ...
    Private Sub B_Delete_Click(sender As Object, e As EventArgs) Handles B_Delete.Click
        ...
        Dim Mcheck As Int32 = O_DV.Find(Mgrade)
        If Mcheck = -1 Then
            MsgBox("Sorry, 『等級』』不存在，不能刪除！" + Chr(13) + Chr(13) + "請重新輸入", 0 + 16, "Error")
            Exit Sub
        End If
        ...
        ' 使用 SqlCommand 更新 SQL Server 資料庫
        ' 宣告變數儲存 SQL 指令
        Dim Msqlstr_1 As String = "Delete From TAB_GRADE Where grade='" + Mgrade + "'"
        ' 以 SqlCommand 建構函式初始化新的執行個體（新物件名為 Ocmd_1），括號內有兩個參數，分別為  SQL 指令 與 連結物件
        Dim Ocmd_1 As New SqlCommand(Msqlstr_1, Ocn_1)
    ...
    Private Sub B_Export_Click(sender As Object, e As EventArgs) Handles B_Export.Click
        ...
        If System.IO.File.Exists("D:\TestQuery\Table_Grade.xls") = True Then
            My.Computer.FileSystem.DeleteFile("D:\TestQuery\Table_Grade.xls")
        End If

        ' 設定連接管道
        Dim MFN_0 As String = "D:\TestQuery\Table_Grade.xls"
        ...
        ' 插入欄名
        Dim Msqlstr_0 As String = "Create Table Sheet1 ([等級] Text(3))"
        ...
        For Mcou = 0 To MRowNo Step 1
            Msqlstr_1 = "Insert Into [" + MSheetName + "] Values(@t1)"
            Ocmd_1.Parameters.Clear()
        ...
        Ocmd_1.Dispose()
        Oconn_0.Close()
        MsgBox("資料已匯出至 D:\TestQuery\Table_Grade.xls！", 0 + 64, "OK")
    End Sub
    ...
    Private Sub B_UpdateSQL_Click(sender As Object, e As EventArgs) Handles B_UpdateSQL.Click
        ...
        ' 宣告變數儲存 SQL 指令
        Dim Msqlstr_1 As String = "Delete From TAB_GRADE"
        ' 以 SqlCommand 建構函式初始化新的執行個體（新物件名為 Ocmd_1），括號內有兩個參數，分別為  SQL 指令 與 連結物件
        Dim Ocmd_1 As New SqlCommand(Msqlstr_1, Ocn_1)
        ...
        ' 宣告變數儲存 SQL 指令
        Msqlstr_1 = "Insert into TAB_GRADE(grade) values(@t1)"
        ' 以 SqlCommand 建構函式初始化新的執行個體（新物件名為 Ocmd_1），括號內有兩個參數，分別為  SQL 指令 與 連結物件
        ...
        ' 第二種方式在 AddWithValue 括號內指定具名參數及料來源
        ' Clear 方法可清除 Parameters
        ' 若 User 匯錯資料時，仍按下『更新』鈕，可能會發生錯誤（例如等級重複了），故加入 Try 陳述式，以處理例外狀況
        Dim Mstop As Int32 = ODataSet_1.Tables("Table01").Rows.Count - 1
