...
Public Class F_TableDept
    ...
    Private Sub F_TableDeptt_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Dim Msqlstr_1 As String = "Select * From TAB_DEPT"
    ...
    Private Sub DataGridView1_SelectionChanged(sender As Object, e As EventArgs) Handles DataGridView1.SelectionChanged
        ...
        T_DeptCode.Text = Strings.Trim(DataGridView1.CurrentRow.Cells(0).Value)
        T_DeptName.Text = Strings.Trim(DataGridView1.CurrentRow.Cells(1).Value)
    End Sub
    ...
    Private Sub B_ADD_Click(sender As Object, e As EventArgs) Handles B_ADD.Click
        ...
        Dim Mdeptcode As String = Strings.Trim(T_DeptCode.Text)
        Dim Mdeptname As String = Strings.Trim(T_DeptName.Text)

        ' 第一段 ************************************************************************************
        ' 檢查『部門代號』、『部門名稱』是否已輸入，此  2 項不得空白
        If Strings.Len(Mdeptcode) <> 4 Then
            MsgBox("『部門代號』未輸入或不正確！" + Chr(13) + Chr(13) + "固定長度 4 Byte", 0 + 16, "Error")
            Exit Sub
        End If
        If Strings.Len(mdeptname) = 0 Then
            MsgBox("『部門名稱』未輸入！", 0 + 16, "Error")
            Exit Sub
        End If

        ' 第二段 ************************************************************************************
        ' 檢查『部門代號』是否已存在，可使用 Find 方法 或 RowFilter 屬性（如下）
        ' 檢查 DataTable 即可（載入本表單時，已將全部資料存入 Table01）

        ' 使用 Find 方法，尋找欄須經排序
        'Dim O_DV As DataView = New DataView(ODataSet_1.Tables("Table01"))
        'O_DV.Sort = "dept_code ASC"
        'Dim Mcheck As Int32 = O_DV.Find(Mdeptcode)
        'If Mcheck <> -1 Then
        'MsgBox("Sorry, 『部門代號』已存在，不能新增相同的代號！" + Chr(13) + Chr(13) + "請重新輸入", 0 + 16, "Error")
        'Exit Sub
        'End If

        ' DataView 檢視表的 RowFilter 屬性篩選出 User 所輸入的部門代號，若有，則表示重複了
        ' 比較值 T_DeptCode.Text 的前後要加單引號
        Dim O_DV As DataView = New DataView(ODataSet_1.Tables("Table01"))
        O_DV.RowFilter = "dept_code='" + Mdeptcode + "'"
        ...
        ' 宣告變數儲存 SQL 指令
        Dim Msqlstr_1 As String = "Insert into TAB_DEPT(dept_code,dept_name) values(@t1,@t2)"
        ...
        Ocmd_1.Parameters.AddWithValue("@t1", SqlDbType.NVarChar).Value = Mdeptcode
        Ocmd_1.Parameters.AddWithValue("@t2", SqlDbType.NVarChar).Value = Mdeptname
        'Ocmd_1.Parameters.AddWithValue("@t1", Mdeptcode)
        'Ocmd_1.Parameters.AddWithValue("@t2", Mdeptname)
        ...
        O_NewRow.Item(0) = Mdeptcode
        O_NewRow.Item(1) = Mdeptname
    ...
    Private Sub B_Modify_Click(sender As Object, e As EventArgs) Handles B_Modify.Click
        ...
        Dim Mcheck As Int32 = O_DV.Find(Mdeptcode)
        If Mcheck = -1 Then
            MsgBox("Sorry, 『部門代號』』不存在，不能修改！" + Chr(13) + Chr(13) + "請重新輸入", 0 + 16, "Error")
            Exit Sub
        End If
        ...
        ' 使用 SqlCommand 更新 SQL Server 資料庫
        ' 宣告變數儲存 SQL 指令
        Dim Msqlstr_1 As String = "Update TAB_DEPT Set dept_name=@t1 Where dept_code='" + Mdeptcode + "'"
        ' 以 SqlCommand 建構函式初始化新的執行個體（新物件名為 Ocmd_1），括號內有兩個參數，分別為  SQL 指令 與 連結物件
        Dim Ocmd_1 As New SqlCommand(Msqlstr_1, Ocn_1)
        ...
        ' Clear 方法可清除 Parameters
        Ocmd_1.Parameters.Clear()
        Ocmd_1.Parameters.AddWithValue("@t1", SqlDbType.NVarChar).Value = Mdeptname
        'Ocmd_1.Parameters.AddWithValue("@t1", Mdeptname)
        ...
        ODataSet_1.Tables("Table01").Rows(Mcheck)(1) = Mdeptname
        ODataSet_1.Tables("Table01").AcceptChanges()
        DataGridView1.DataSource = Nothing
        DataGridView1.DataSource = ODataSet_1.Tables("Table01")
    ...
    Private Sub B_Delete_Click(sender As Object, e As EventArgs) Handles B_Delete.Click
        ...
        Dim Mcheck As Int32 = O_DV.Find(Mdeptcode)
        If Mcheck = -1 Then
            MsgBox("Sorry, 『部門代號』』不存在，不能刪除！" + Chr(13) + Chr(13) + "請重新輸入", 0 + 16, "Error")
            Exit Sub
        End If
        ...
        ' 使用 SqlCommand 更新 SQL Server 資料庫
        ' 宣告變數儲存 SQL 指令
        Dim Msqlstr_1 As String = "Delete From TAB_DEPT Where dept_code='" + Mdeptcode + "'"
        ' 以 SqlCommand 建構函式初始化新的執行個體（新物件名為 Ocmd_1），括號內有兩個參數，分別為  SQL 指令 與 連結物件
        Dim Ocmd_1 As New SqlCommand(Msqlstr_1, Ocn_1)
    ...
    Private Sub B_Export_Click(sender As Object, e As EventArgs) Handles B_Export.Click
        ...
        If System.IO.File.Exists("D:\TestQuery\Table_Dept.xls") = True Then
            My.Computer.FileSystem.DeleteFile("D:\TestQuery\Table_Dept.xls")
        End If

        ' 設定連接管道
        Dim MFN_0 As String = "D:\TestQuery\Table_Dept.xls"
        ...
        ' 插入欄名
        Dim Msqlstr_0 As String = "Create Table Sheet1 ([部門代號] Text(4), [部門名稱] Text(10))"
        ...
        Ocmd_1.Dispose()
        Oconn_0.Close()
        MsgBox("資料已匯出至 D:\TestQuery\Table_Dept.xls！", 0 + 64, "OK")
    End Sub
    ...
    Private Sub B_Import_Click(sender As Object, e As EventArgs) Handles B_Import.Click
        ...
        ' 例如新增資料在 DataGridView 中不存在，但在 SQL Server 中已存在，就會發生錯誤，故此時應隱藏增刪修按鈕，直到 User 按下『更新』鈕為止
        B_ADD.Visible = False
    ...
    Private Sub B_UpdateSQL_Click(sender As Object, e As EventArgs) Handles B_UpdateSQL.Click
        ...
        ' 宣告變數儲存 SQL 指令
        Dim Msqlstr_1 As String = "Delete From TAB_DEPT"
        ' 以 SqlCommand 建構函式初始化新的執行個體（新物件名為 Ocmd_1），括號內有兩個參數，分別為  SQL 指令 與 連結物件
        Dim Ocmd_1 As New SqlCommand(Msqlstr_1, Ocn_1)
        ...
        ' 宣告變數儲存 SQL 指令
        Msqlstr_1 = "Insert into TAB_DEPT(dept_code,dept_name) values(@t1,@t2)"
        ' 以 SqlCommand 建構函式初始化新的執行個體（新物件名為 Ocmd_1），括號內有兩個參數，分別為  SQL 指令 與 連結物件
